...
function renderFilteredProjects(filteredProjects, containerElement, headingLevel = 'h2') {
  ...
  for (let i = 0; i < arcs.length; i++) {
    ...
    // Append the path to the SVG using appendChild
    const svgElement = d3.select('svg').node(); // Get the raw DOM element
    svgElement.appendChild(path); // Use native appendChild
  }
